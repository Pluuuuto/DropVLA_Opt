...
def get_checkpoint_lambda_tag(cfg: GenerateConfig) -> Optional[str]:
    """Extract a filesystem-safe ``lambdaXpY`` tag from the checkpoint name."""
    checkpoint_name = Path(str(cfg.pretrained_checkpoint).rstrip("/")).name
    lambda_match = re.search(r"lambda(?P<value>[0-9]+p[0-9]+)", checkpoint_name)
    if lambda_match is None:
        return None
    return f"lambda{lambda_match.group('value')}"


def get_rollout_subdir(cfg: GenerateConfig) -> str:
...
def log_hyperparameters(cfg: GenerateConfig, log_file):
    ...
    log_message(f"  Rollout Subdirectory: {get_rollout_subdir(cfg)}", log_file)
...
def setup_logging(cfg: GenerateConfig):
    ...
    lambda_tag = get_checkpoint_lambda_tag(cfg)
    ...
    for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
        ...
        if total_episodes % 10 == 0:
            ...
            save_rollout_video(
                ...
                rollout_subdir=get_rollout_subdir(cfg),
            )
