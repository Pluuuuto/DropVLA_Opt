...
if [[ -z "$CKPT" ]]; then
    ...
    echo "Example:"
    echo "  CKPT=\"$ROOT/RUN/<final-vl0p31-checkpoint>\" GPU_ID=4 TRIALS=1 $0 joint"
    exit 2
fi
...
LOG_DIR="$ROOT/experiments/logs/vl0p31"
mkdir -p "$LOG_DIR"
...
python experiments/robot/libero/run_libero_eval.py \
  ...
  --run_id_note "vl0p31-${MODE}-trial${TRIALS}-seed${SEED}" \
  --use_wandb False \
  --seed "$SEED"
